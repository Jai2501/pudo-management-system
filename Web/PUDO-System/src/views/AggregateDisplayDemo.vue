    ...
    <div class="container">
        ...
        <div class="right">
            <div class="advertisement">
                <img class="advertisement-image" src="../assets/Advertisements/AdvertisementStill.png" alt="">
            </div>
        </div>
    ...
    methods: {
            ...
            switch(berthToBeUpdated) {
                case "A1":
                    ...
                    update(ref(db, 'berth-live-info'), {
                        'berth-a-1-time': new Date().getTime() + this.millisecondsInThreeMinutes
                    })
                    break;
                case "A2":
                    update(ref(db, 'berth-live-info'), {
                        'berth-a-2-time': new Date().getTime() + this.millisecondsInThreeMinutes
                    })
                    break;
                case "A3":
                    update(ref(db, 'berth-live-info'), {
                        'berth-a-3-time': new Date().getTime() + this.millisecondsInThreeMinutes
                    })
                    break;
                case "Priority":
                    update(ref(db, 'berth-live-info'), {
        ...
        async setBerthSampleData() {
            ...
            set(ref(db, 'berth-live-info'), {
                ...
                'berth-a-2-time': (new Date().getTime() + this.millisecondsInThreeMinutes),
                'berth-a-3-time': (new Date().getTime() + this.millisecondsInThreeMinutes),
                'berth-a-4-time': (new Date().getTime() + this.millisecondsInThreeMinutes),
                'berth-pwd-time': (new Date().getTime() + this.millisecondsInFiveMinutes)

                // 'berth-a-1-time': (new Date().getTime() + this.millisecondsInThreeMinutes - Math.floor((Math.random() * 100000))),
                // 'berth-a-2-time': (new Date().getTime() + this.millisecondsInThreeMinutes - Math.floor((Math.random() * 100000))),
                // 'berth-a-3-time': (new Date().getTime() + this.millisecondsInThreeMinutes - Math.floor((Math.random() * 100000))),
                // 'berth-a-4-time': (new Date().getTime() + this.millisecondsInThreeMinutes - Math.floor((Math.random() * 100000))),
                // 'berth-pwd-time': (new Date().getTime() + this.millisecondsInFiveMinutes - Math.floor((Math.random() * 100000)))
            })
        }
    },
    props: [],
